...
describe('RMRKTypedMultiAssetMock', async () => {
  ...
  describe('RMRKTypedMultiAssetMock get top assets', async () => {
    ...
    it('can get top asset by priority and type', async () => {
      await typedMultiAsset.addTypedAssetEntry(
        resId,
        'ipfs://res1.jpg',
        'image/jpeg',
      );
      ...
      expect(
        await typedMultiAsset.getTopAssetMetaForTokenWithType(
          tokenId,
          'image/jpeg',
        ),
    ...
    it('cannot get top asset for if token has no assets with this type', async () => {
      await typedMultiAsset.addTypedAssetEntry(
        resId,
        'ipfs://res1.jpg',
        'image/jpeg',
      ...
      await expect(
        typedMultiAsset.getTopAssetMetaForTokenWithType(
          tokenId,
          'application/pdf',
        ),
...
describe('RMRKNestableTypedMultiAssetMock', async () => {
  let typedNestableMultiAsset: RMRKNestableTypedMultiAssetMock;

  beforeEach(async function () {
    ({ typedNestableMultiAsset } = await loadFixture(
      nestableTypedMultiAssetFixture,
    ));
...
describe('RMRKTypedEquippableMock', async () => {
...
async function shouldBehaveLikeTypedMultiAssetInterface() {
  ...
  it('can support IRMRKTypedMultiAsset', async function () {
    expect(await this.assets.supportsInterface(IRMRKTypedMultiAsset)).to.equal(
      true,
    );
  });

  it('does not support other interfaces', async function () {
    expect(await this.assets.supportsInterface(IOtherInterface)).to.equal(
      false,
    );
  });
}
