...
describe('RMRKMultiAssetAutoIndexMock', async () => {
  ...
  it('can support IERC5773', async () => {
    expect(await token.supportsInterface(IERC5773)).to.equal(true);
  });

  it('can support IRMRKMultiAssetAutoIndex', async () => {
    expect(await token.supportsInterface(IRMRKMultiAssetAutoIndex)).to.equal(
      true,
    );
  });
  ...
  describe('With minted tokens', async () => {
    ...
    it('can accept an asset', async () => {
      await token
        .connect(user)
        ['acceptAsset(uint256,uint64)'](tokenId, assetId);
      expect(await token.getPendingAssets(tokenId)).to.be.eql([]);
      expect(await token.getActiveAssets(tokenId)).to.be.eql([assetId]);
    });

    it('can reject an asset', async () => {
      await token
        .connect(user)
        ['rejectAsset(uint256,uint64)'](tokenId, assetId);
      expect(await token.getPendingAssets(tokenId)).to.be.eql([]);
      expect(await token.getActiveAssets(tokenId)).to.be.eql([]);
    });
  });

  describe('With multiple assets in the pending list', async () => {
    ...
    beforeEach(async () => {
      ...
      await token
        .connect(owner)
        .addAssetEntry(assetThreeId, 'ipfs/asset3.json');
    ...
    it('can reject a middle asset in the pending list', async () => {
      await token
        .connect(user)
        ['rejectAsset(uint256,uint64)'](tokenId, assetTwoId);
      expect(await token.getPendingAssets(tokenId)).to.be.eql([bn(1), bn(3)]);
      expect(await token.getActiveAssets(tokenId)).to.be.eql([]);
    });

    it('can accept a middle asset in the pending list', async () => {
      await token
        .connect(user)
        ['acceptAsset(uint256,uint64)'](tokenId, assetTwoId);
      expect(await token.getPendingAssets(tokenId)).to.be.eql([
        asseOneId,
        assetThreeId,
      ]);
    ...
    it('can reject first asset in the pending list', async () => {
      await token
        .connect(user)
        ['rejectAsset(uint256,uint64)'](tokenId, asseOneId);
      expect(await token.getPendingAssets(tokenId)).to.be.eql([
        assetThreeId,
        assetTwoId,
      ]);
    ...
    it('can accept first asset in the pending list', async () => {
      await token
        .connect(user)
        ['acceptAsset(uint256,uint64)'](tokenId, asseOneId);
      expect(await token.getPendingAssets(tokenId)).to.be.eql([
        assetThreeId,
        assetTwoId,
      ]);
    ...
    it('can reject all assets in the pending list', async () => {
      await token
        .connect(user)
        ['rejectAsset(uint256,uint64)'](tokenId, asseOneId);
      await token
        .connect(user)
        ['rejectAsset(uint256,uint64)'](tokenId, assetThreeId);
      await token
        .connect(user)
        ['rejectAsset(uint256,uint64)'](tokenId, assetTwoId);
      expect(await token.getPendingAssets(tokenId)).to.be.eql([]);
    ...
    it('can accept all assets in the pending list', async () => {
      await token
        .connect(user)
        ['acceptAsset(uint256,uint64)'](tokenId, asseOneId);
      await token
        .connect(user)
        ['acceptAsset(uint256,uint64)'](tokenId, assetThreeId);
      await token
        .connect(user)
        ['acceptAsset(uint256,uint64)'](tokenId, assetTwoId);
      expect(await token.getPendingAssets(tokenId)).to.be.eql([]);
    ...
    describe('With multiple assets in the active list', async () => {
      ...
      it('can replace the first active asset', async () => {
        await token
          .connect(owner)
          .addAssetEntry(assetFourId, 'ipfs/asset4.json');
        await token
          .connect(owner)
          .addAssetToToken(tokenId, assetFourId, asseOneId);
        await token
          .connect(user)
          ['acceptAsset(uint256,uint64)'](tokenId, assetFourId);
        expect(await token.getPendingAssets(tokenId)).to.be.eql([]);
        expect(await token.getActiveAssets(tokenId)).to.be.eql([
          assetFourId,
          assetThreeId,
