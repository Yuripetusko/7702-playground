...
describe('RMRKNestableClaimableChildMock', async () => {
  beforeEach(async function () {
    const { parent, child } = await loadFixture(
      reclaimableChildNestableFixture,
    );
    this.parent = parent;
    this.child = child;
  });
...
async function shouldBehaveLikeReclaimableChild() {
  ...
  beforeEach(async function () {
    ...
    childId = await nestMintFromMock(
      this.child,
  ...
  it('can support IRMRKReclaimableChild', async function () {
    expect(await this.parent.supportsInterface(IRMRKReclaimableChild)).to.equal(
      true,
    );
  });
  ...
  it('does not support other interfaces', async function () {
    expect(await this.parent.supportsInterface(IOtherInterface)).to.equal(
      false,
    );
  });

  describe('With active child', async () => {
    beforeEach(async function () {
      await this.parent
        .connect(tokenOwner)
  ...
  describe('With pending child', async () => {
    it('can reclaim transferred child if transferred to address zero', async function () {
      await this.parent
        .connect(tokenOwner)
        .transferChild(
