...
describe('CatalogImpl', async () => {
  ...
  describe('With added parts', async () => {
    ...
    it('can get total parts', async () => {
      expect(await catalog.getTotalParts()).to.eql(5n);
    });

    it('can get part by index', async () => {
      expect(await catalog.getPartByIndex(0)).to.eql([
        fixedType,
        0n,
        [],
        'ipfs://metadata1',
      ]);
      expect(await catalog.getPartByIndex(4)).to.eql([
        fixedType,
        0n,
        [],
        'ipfs://metadata5',
    ...
    it('can get all part ids', async () => {
      expect(await catalog.getAllPartIds()).to.eql([1n, 2n, 3n, 4n, 5n]);
    });

    it('can get paginated part ids', async () => {
    ...
    it.skip('can get all part ids up to 10k, skipped so tests run faster', async () => {
      ...
      for (let i = 0; i < partList.length; i += chunkSize) {
        await catalog
          .connect(owner)
          .addPartList(partList.slice(i, i + chunkSize));
      }
  ...
  describe('Interface and Permissions', async () => {
    ...
    it('supports catalog interface', async () => {
      expect(await catalog.supportsInterface(IRMRKCatalogExtended)).to.equal(
        true,
      );
    });

    it('cannot do admin operations if not owner or contributor', async () => {
      await expect(
      ...
      await expect(
        catalog
          .connect(other)
          .addPartList([{ partId: partId, part: partData }]),
      ).to.be.revertedWithCustomError(catalog, 'RMRKNotOwnerOrContributor');
      ...
      await expect(
        catalog.connect(other).setEquippableToAll(partId),
      ).to.be.revertedWithCustomError(catalog, 'RMRKNotOwnerOrContributor');
    ...
    it('cannot add parts if locked', async () => {
    ...
    it('can add part if owner', async () => {
      await catalog.connect(owner).addPart({ partId: partId, part: partData });
      expect(await catalog.getPart(partId)).to.eql([
        fixedType,
        0n,
        [],
        'ipfs://metadata',
    ...
    it('can add part if contributor', async () => {
      await catalog.connect(owner).manageContributor(contributor.address, true);
      await catalog
        .connect(contributor)
        .addPart({ partId: partId, part: partData });
      expect(await catalog.getPart(partId)).to.eql([
        fixedType,
        0n,
        [],
    ...
    it('can set metadataURI or type if owner', async () => {
    ...
    it('can set metadataURI or type if contributor', async () => {
      await catalog.connect(owner).manageContributor(contributor.address, true);
    ...
    it('cannot set metadataURI nor type if not owner or contributor', async () => {
      await expect(
        catalog.connect(other).setMetadataURI('ipfs://new'),
      ).to.be.revertedWithCustomError(catalog, 'RMRKNotOwnerOrContributor');
      await expect(
        catalog.connect(other).setType('img/png'),
      ).to.be.revertedWithCustomError(catalog, 'RMRKNotOwnerOrContributor');
    });
  });
});
