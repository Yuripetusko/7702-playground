...
describe('CatalogImpl', async () => {
  ...
  it('can deploy a new catalog', async () => {
    ...
    const catalog = await ethers.getContractAt(
      'RMRKCatalogImpl',
      catalogAddress,
    );
  ...
  it('can get catalogs deployed by a deployer', async () => {
    ...
    expect(
      await catalogFactory.getDeployerCatalogs(deployer1.address),
    ).to.deep.equal([catalogAddress1, catalogAddress2]);
    expect(
      await catalogFactory.getDeployerCatalogs(deployer2.address),
    ).to.deep.equal([catalogAddress3]);

    expect(
      await catalogFactory.getLastDeployerCatalog(deployer1.address),
    ).to.equal(catalogAddress2);
    expect(
      await catalogFactory.getLastDeployerCatalog(deployer2.address),
    ).to.equal(catalogAddress3);

    expect(
      await catalogFactory.getTotalDeployerCatalogs(deployer1.address),
    ).to.equal(2);
    expect(
      await catalogFactory.getTotalDeployerCatalogs(deployer2.address),
    ).to.equal(1);

    expect(
      await catalogFactory.getDeployerCatalogAtIndex(deployer1.address, 0),
    ).to.equal(catalogAddress1);
    expect(
      await catalogFactory.getDeployerCatalogAtIndex(deployer1.address, 1),
    ).to.equal(catalogAddress2);
  });
  ...
  async function deployAndGetAddress(
    ...
    const tx = await catalogFactory
      .connect(deployer)
      .deployCatalog(metadataURI, mediaType);
    const receipt = await tx.wait();
    const catalogAddress = receipt?.logs?.[0]?.address;
    if (!catalogAddress) {
      throw new Error('Catalog address not found');
