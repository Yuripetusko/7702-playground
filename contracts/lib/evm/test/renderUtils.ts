...
async function multiAsetNestableAndEquipRenderUtilsFixture() {
  ...
  const renderUtilsNestableFactory = await ethers.getContractFactory(
    'RMRKNestableRenderUtils',
  );
  ...
  const renderUtilsNestable = <RMRKNestableRenderUtils>(
    await renderUtilsNestableFactory.deploy()
  );
  await renderUtilsNestable.waitForDeployment();

  const renderUtilsEquip = <RMRKEquipRenderUtils>(
...
describe('MultiAsset Nestable and Equip Render Utils', async () => {
  ...
  beforeEach(async () => {
    ({ catalog, equip, renderUtils, renderUtilsNestable, renderUtilsEquip } =
      await loadFixture(multiAsetNestableAndEquipRenderUtilsFixture));
  ...
  describe('Render Utils MultiAsset', async () => {
    it('can get active assets', async () => {
      expect(
        await renderUtils.getExtendedActiveAssets(
    ...
    it('can get assets by id', async () => {
      expect(
        await renderUtils.getAssetsById(await equip.getAddress(), tokenId, [
          resId,
          resId2,
        ]),
    ...
    it('can get pending assets', async () => {
      expect(
        await renderUtils.getPendingAssets(await equip.getAddress(), tokenId),
      ).to.eql([
    ...
    it('can get top asset by priority', async () => {
      expect(
        await renderUtils.getTopAssetMetaForToken(
          await equip.getAddress(),
          tokenId,
        ),
    ...
    it('can get full top asset data', async () => {
      expect(
        await renderUtils.getTopAsset(await equip.getAddress(), tokenId),
      ).to.eql([resId2, bn(5), 'ipfs://res2.jpg']);
    });

    it('cannot get active assets if token has no assets', async () => {
      ...
      await expect(
        renderUtilsEquip.getExtendedEquippableActiveAssets(
          await equip.getAddress(),
          otherTokenId,
        ),
      ).to.be.revertedWithCustomError(renderUtils, 'RMRKTokenHasNoAssets');
    ...
    it('cannot get pending assets if token has no assets', async () => {
      const otherTokenId = await mintFromMock(equip, owner.address);
      await expect(
    ...
    it('cannot get top asset if token has no assets', async () => {
      const otherTokenId = await mintFromMock(equip, owner.address);
      await expect(
  ...
  describe('Render Utils Nestable', async () => {
    let parentId: bigint;
    let childId1: bigint;
    let childId2: bigint;
    ...
    beforeEach(async () => {
      ...
      childId1 = await nestMintFromMock(
        equip,
        await equip.getAddress(),
        parentId,
      );
      childId2 = await nestMintFromMock(
        equip,
        await equip.getAddress(),
        parentId,
      );
    ...
    it('can get total descendants', async () => {
      expect(
        await renderUtilsNestable.getTotalDescendants(
          await equip.getAddress(),
          parentId,
        ),
      ).to.eql([bn(2), false]);

      const grandChildId = await nestMintFromMock(
        equip,
        await equip.getAddress(),
        childId1,
      );
      await equip.acceptChild(
        childId1,
        0,
        await equip.getAddress(),
        grandChildId,
    ...
    it('can tell if a token has multiple levels of nesting', async () => {
      expect(
        await renderUtilsNestable.hasMoreThanOneLevelOfNesting(
          await equip.getAddress(),
          parentId,
        ),
      ).to.eql(false);
  ...
  describe('Render Utils Equip', async () => {
    it('can get active assets', async () => {
      expect(
        await renderUtilsEquip.getExtendedEquippableActiveAssets(
          await equip.getAddress(),
          tokenId,
        ),
      ).to.eql([
    ...
    it('can get pending assets', async () => {
      expect(
        await renderUtilsEquip.getExtendedPendingAssets(
          await equip.getAddress(),
          tokenId,
        ),
      ).to.eql([
    ...
    it('can get top equippable data for asset by priority', async () => {
      expect(
    ...
    it('cannot get equippable slots from parent if parent is not an NFT', async () => {
      await expect(
...
describe('Advanced Equip Render Utils', async () => {
  let owner: SignerWithAddress;
  let catalog: RMRKCatalogImpl;
  ...
  it('can get equipped children from parent', async () => {
  ...
  it('can get equippable slots from parent', async () => {
    await setUpCatalog(catalog, await gem.getAddress());
    await setUpKanariaAsset(kanaria, kanariaId, await catalog.getAddress());
    await setUpGemAssets(
      gem,
    ...
    expect(
      await renderUtilsEquip.getAllEquippableSlotsFromParent(
        await gem.getAddress(),
        gemId3,
        false,
      ),
  ...
  it('can get children with top metadata', async () => {
    await setUpCatalog(catalog, await gem.getAddress());
    await setUpKanariaAsset(kanaria, kanariaId, await catalog.getAddress());
    await setUpGemAssets(
      gem,
    ...
    expect(
      await renderUtilsEquip.getChildrenWithTopMetadata(
        await kanaria.getAddress(),
        kanariaId,
      ),
    ).to.eql([
    ...
    expect(
      await renderUtilsEquip.getTopAssetMetadataForTokens(
        await gem.getAddress(),
        [gemId1, gemId2, gemId3],
      ),
    ).to.eql([
      'ipfs://gems/typeA/left.svg',
  ...
  it('can get equippable slots from parent for pending child', async () => {
    await setUpCatalog(catalog, await gem.getAddress());
    await setUpKanariaAsset(kanaria, kanariaId, await catalog.getAddress());
    await setUpGemAssets(
      gem,
  ...
  it('can get equippable slots from parent asset even if catalog does not match', async () => {
    await setUpCatalog(catalog, await gem.getAddress());
    await setUpKanariaAsset(kanaria, kanariaId, await catalog.getAddress());
    await setUpGemAssets(
      gem,
  ...
  it('cannot get equippable slots from parent if the asset id is not composable', async () => {
  ...
  it('fails checking expected parent if parent is not the expected one', async () => {
    await expect(
  ...
  it('succeeds checking expected parent if parent is the expected one', async () => {
    await expect(
      renderUtilsEquip.checkExpectedParent(
        await gem.getAddress(),
        gemId1,
...
describe('Extended NFT render utils', () => {
  ...
  it('renders correct data for MultiAsset', async () => {
  ...
  it('renders correct data for soulbound Nestable', async () => {
  ...
  it('renders correct data for Nestable with MultiAsset', async () => {
    const parentId = await mintFromMockPremint(
      nestableMultiAsset,
      rootOwner.address,
    );
    const tokenId = await mintFromMockPremint(
  ...
  it('renders correct data for Equippable', async () => {
    ...
    await equip
      .connect(rootOwner)
      .acceptChild(tokenId, 0, await equip.getAddress(), child1);
    await equip
      .connect(rootOwner)
      .acceptChild(tokenId, 0, await equip.getAddress(), child3);
    await equip
      .connect(rootOwner)
      .nestTransferFrom(
        rootOwner.address,
    ...
    await equip
      .connect(rootOwner)
      .acceptChild(parentId, 0, await equip.getAddress(), tokenId);
  ...
  describe('Nesting validation', () => {
    ...
    beforeEach(async () => {
      parentTokenOne = await mintFromMockPremint(
        nestableMultiAsset,
        rootOwner.address,
      );
      parentTokenTwo = await mintFromMockPremint(
        nestableMultiAsset,
        rootOwner.address,
      );
    ...
    it('returns true if the specified token is nested into the given parent', async () => {
      expect(
    ...
    it('returns false if the child does not implement IERC7401', async () => {
      expect(
        await renderUtils.validateChildOf(
          await nestableMultiAsset.getAddress(),
          await multiAsset.getAddress(),
    ...
    it('returns false if the specified child token is not the child token of the parent token', async () => {
      expect(
        await renderUtils.validateChildOf(
          await nestableMultiAsset.getAddress(),
          await nestableMultiAsset.getAddress(),
    ...
    it('returns true if the specified children are the child tokens of the given parent token', async () => {
      expect(
    ...
    it('does not allow to pass different length child token address and token ID arrays', async () => {
      await expect(
    ...
    it('returns false if one of the child tokens does not implement IERC7401', async () => {
      expect(
        await renderUtils.validateChildrenOf(
          await nestableMultiAsset.getAddress(),
          [
    ...
    it('returns false if any of the given tokens is not owned by the specified parent token', async () => {
      expect(
        await renderUtils.validateChildrenOf(
          await nestableMultiAsset.getAddress(),
          [
    ...
    it('can get directOwnerOf with parents perspective', async () => {
      expect(
    ...
    it('can identify rejected children', async () => {
      expect(
    ...
    it('can identify abandoned children', async () => {
      await nestableMultiAsset
        .connect(rootOwner)
        .acceptChild(
          parentTokenOne,
