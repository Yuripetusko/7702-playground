...
import { IERC165, IERC7409, IOtherInterface } from './interfaces';
...
describe('RMRKEmotesRepository', async () => {
  ...
  describe('With minted tokens', async () => {
    ...
    it('can undo emote', async () => {
      await repository.emote(await token.getAddress(), tokenId, emoji1, true);

      await expect(
        repository.emote(await token.getAddress(), tokenId, emoji1, false),
      )
        .to.emit(repository, 'Emoted')
        .withArgs(
          owner.address,
    ...
    it('can be emoted from different accounts', async () => {
      await repository
        .connect(addrs[0])
        .emote(await token.getAddress(), tokenId, emoji1, true);
      await repository
    ...
    it('can add multiple emojis to same NFT', async () => {
      await repository.emote(await token.getAddress(), tokenId, emoji1, true);
    ...
    it('does nothing if new state is the same as old state', async () => {
      await repository.emote(await token.getAddress(), tokenId, emoji1, true);
      await repository.emote(await token.getAddress(), tokenId, emoji1, true);
      expect(
        await repository.emoteCountOf(
    ...
    it('does nothing if new state is the same as old state on bulk emote', async () => {
      await repository.emote(await token.getAddress(), tokenId, emoji1, true);
    ...
    it('can bulk emote and unemote at the same time', async () => {
      await repository.emote(await token.getAddress(), tokenId, emoji2, true);
    ...
    it('can not bulk emote if passing arrays of different length', async () => {
      await expect(
        repository.bulkEmote(
          [await token.getAddress(), await token.getAddress()],
          [tokenId, tokenId],
    ...
    it('can not bulk presign if passing arrays of different length', async () => {
      await expect(
    ...
    it('can not bulk get emote count if passing arrays of different length', async () => {
      await expect(
    ...
    it('can not bulk check for emotes if passing arrays of different length', async () => {
      await expect(
    ...
    it('can use presigned emote to react to token', async () => {
    ...
    it('does nothing if new state is the same as old state on presigned emote', async () => {
      repository
        .connect(owner)
        .emote(await token.getAddress(), tokenId, emoji1, true);
      const message = await repository.prepareMessageToPresignEmote(
    ...
    it('can use presigned emote to undo reaction to token', async () => {
      repository
        .connect(owner)
        .emote(await token.getAddress(), tokenId, emoji1, true);
      const message = await repository.prepareMessageToPresignEmote(
    ...
    it('cannot use expired presigned to emote', async () => {
    ...
    it('cannot use presigned emote for a different signer, collection, token, emoji, state or deadline', async () => {
      const message = await repository.prepareMessageToPresignEmote(
        await token.getAddress(),
        tokenId,
        emoji1,
    ...
    it('can use presigned emotes to bulk react to token', async () => {
    ...
    it('can use presigned emotes to undo reaction to token', async () => {
      repository
        .connect(owner)
        .emote(await token.getAddress(), tokenId, emoji1, true);
      repository
    ...
    it('does nothing if new state is the same as old state on bulk presigned emote', async () => {
      repository
        .connect(owner)
        .emote(await token.getAddress(), tokenId, emoji1, true);
    ...
    it('can use bulk presigned emotes if passing arrays of different length', async () => {
      const messages = await repository.bulkPrepareMessagesToPresignEmote(
        [await token.getAddress(), await token.getAddress()],
        [tokenId, tokenId],
        [emoji1, emoji2],
    ...
    it('cannot use expired presigned to bulk emote', async () => {
      const block = await ethers.provider.getBlock('latest');
      if (!block) {
        throw new Error('Block not found');
      }
    ...
    it('cannot use bulk presigned emote for a different signer, collection, token, emoji, state or deadline', async () => {
      const block = await ethers.provider.getBlock('latest');
      if (!block) {
        throw new Error('Block not found');
      }
