...
describe('RMRKRoyaltiesSplitter', () => {
  ...
  beforeEach(async () => {
    [sender, beneficiary1, beneficiary2, beneficiary3] =
      await ethers.getSigners();
    beneficiaries = [beneficiary1, beneficiary2, beneficiary3];

    const royaltiesSplitterFactory = await ethers.getContractFactory(
      'RMRKRoyaltiesSplitter',
    );
    royaltiesSplitter = await royaltiesSplitterFactory.deploy(
      beneficiaries.map(async (b) => b.address),
      SHARES_BPS,
    );
  ...
  it('should distribute native payment correctly', async () => {
    ...
    await expect(async () =>
      sender.sendTransaction({
        to: await royaltiesSplitter.getAddress(),
        value: amount,
      }),
  ...
  it('should distribute ERC20 payment correctly', async () => {
    ...
    await erc20Token
      .connect(sender)
      .transfer(await royaltiesSplitter.getAddress(), amount);
    ...
    expect(
      ...
      .withArgs(sender.address, await erc20Token.getAddress(), amount);
    expect(await erc20Token.balanceOf(beneficiary1.address)).to.equal(
      (amount * 2500n) / 10000n,
    );
    expect(await erc20Token.balanceOf(beneficiary2.address)).to.equal(
      (amount * 2500n) / 10000n,
    );
    expect(await erc20Token.balanceOf(beneficiary3.address)).to.equal(
      (amount * 5000n) / 10000n,
    );
  });
  ...
  it('cannot create with invalid configuration', async () => {
    ...
    await expect(
      royaltiesSplitterFactory.deploy(
        beneficiaries.slice(0, 2).map(async (b) => b.address),
        SHARES_BPS,
      ),
    ).to.be.reverted;
    // Shares below 10000
