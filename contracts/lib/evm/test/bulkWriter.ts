...
describe('Advanced Equip Render Utils', async () => {
  ...
  describe('With General Bulk Writer', async () => {
    describe('Bulk Equip', async () => {
      beforeEach(async () => {
      ...
      it('can unequip and equip in bulk', async () => {
      ...
      it('can use bulk with only unequip operations', async () => {
        // On a single call we remove the gem from the first slot and add 2 gems on the other 2 slots
        await bulkWriter.bulkEquip(
          kanariaAddress,
          kanariaId,
      ...
      it('can use bulk with only equip operations', async () => {
        // On a single call we remove the gem from the first slot and add 2 gems on the other 2 slots
        await bulkWriter.bulkEquip(
          kanariaAddress,
          kanariaId,
      ...
      it('cannot do operations if not writer is not approved', async () => {
      ...
      it('cannot do operations if not token owner', async () => {
      ...
      it('cannot do operations for if token id on equip data, does not match', async () => {
        const otherId = 2;
        await expect(
          bulkWriter.bulkEquip(
            kanariaAddress,
    ...
    describe('Bulk Child Transfer', async () => {
      beforeEach(async () => {
        await kanaria.setApprovalForAll(bulkWritterAddress, true);
      });

      it('can transfer children in bulk', async () => {
        await bulkWriter.bulkTransferChildren(
          kanariaAddress,
          kanariaId,
          [0, 2],
          owner.address,
        ...
        expect((await gem.directOwnerOf(gemId1)).owner_).to.equal(
          owner.address,
        );
        expect((await gem.directOwnerOf(gemId2)).owner_).to.equal(
          kanariaAddress,
        );
        expect((await gem.directOwnerOf(gemId3)).owner_).to.equal(
          owner.address,
        );
      });

      it('can transfer all children in bulk', async () => {
        await bulkWriter.bulkTransferAllChildren(
          kanariaAddress,
          kanariaId,
          owner.address,
          0,
      ...
      it('can transfer all children in bulk and it ignores if soulbound', async () => {
        const soulboundFactory = await ethers.getContractFactory(
        ...
        expect(
          (await soulbound.directOwnerOf(soulboundTokenId)).owner_,
        ).to.equal(kanariaAddress);
        expect((await gem.directOwnerOf(gemId1)).owner_).to.equal(
          owner.address,
        );
        expect((await gem.directOwnerOf(gemId2)).owner_).to.equal(
      ...
      it('cannot do operations if not writer is not approved', async () => {
        ...
        // On a single call we remove the gem from the first slot and add 2 gems on the other 2 slots
        await expect(
          bulkWriter.bulkTransferChildren(
            kanariaAddress,
            kanariaId,
            [0, 2],
            owner.address,
        ...
        await expect(
          bulkWriter.bulkTransferAllChildren(
            kanariaAddress,
            kanariaId,
            owner.address,
            0,
  ...
  describe('With Bulk Writer Per Collection', async () => {
    beforeEach(async () => {
      await kanaria.setApprovalForAllForAssets(
        await bulkWriterPerCollection.getAddress(),
        true,
      );
    ...
    it('can get managed collection', async () => {
      expect(await bulkWriterPerCollection.getCollection()).to.equal(
        kanariaAddress,
      );
    });
