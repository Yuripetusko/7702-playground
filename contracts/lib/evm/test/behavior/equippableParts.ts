...
async function shouldBehaveLikeEquippableWithParts() {
  ...
  describe('Equip', async () => {
    it('can equip weapon', async () => {
      ...
      expect(
        await viewContract.getEquipped(
      ...
      expect(
        await neonContract.isChildEquipped(
          neons[0],
          await maskContract.getAddress(),
          masks[0],
        ),
    ...
    it('cannot equip non existing child in slot', async () => {
  ...
  describe('Compose', async () => {
    it('can compose all parts for neon', async () => {
      const childIndex = 0;
      const weaponResId = maskAssetsEquip[0]; // This asset is assigned to weapon first weapon
    ...
    it('can compose all parts for mask', async () => {
      const expectedFixedParts = [
        [
    ...
    it('cannot compose equippables for neon with not associated asset', async () => {
    ...
    it('cannot compose equippables for mask for asset with no catalog', async () => {
      ...
      await maskContract
        .connect(addrs[0])
        .acceptAsset(masks[0], 0, noCatalogAssetId);
      await expect(
        viewContract.composeEquippables(
          await maskContract.getAddress(),
          masks[0],
