...
import { GenericCatalog, GenericEquippable, bn } from '../utils';
...
async function shouldBehaveLikeEquippableWithSlots(
  nestMint: (
    token: GenericEquippable,
    to: string,
    parentId: bigint,
  ) => Promise<bigint>,
) {
  let catalog: GenericCatalog;
  let soldier: GenericEquippable;
  let weapon: GenericEquippable;
  let weaponGem: GenericEquippable;
  let background: GenericEquippable;
  let view: RMRKEquipRenderUtils;
  ...
  describe('Validations', async () => {
    it('can validate equips of weapons into soldiers', async () => {
    ...
    it('can validate equips of weapon gems into weapons', async () => {
      // This asset is not equippable
      expect(
    ...
    it('can validate equips of backgrounds into soldiers', async () => {
  ...
  describe('Equip', async () => {
    ...
    it('can equip weapon if approved', async () => {
      // Weapon is child on index 0, background on index 1
      const soldierOwner = addrs[0];
      ...
      await soldier
        ...
        .approveForAssets(approved.address, soldiersIds[0]);
      await equipWeaponAndCheckFromAddress(approved, childIndex, weaponResId);
    });

    it('can equip weapon if approved for all', async () => {
      // Weapon is child on index 0, background on index 1
      const soldierOwner = addrs[0];
      const approved = addrs[1];
      const childIndex = 0;
      ...
      await soldier
        .connect(soldierOwner)
        .setApprovalForAllForAssets(approved.address, true);
      await equipWeaponAndCheckFromAddress(approved, childIndex, weaponResId);
    });

    it('can equip weapon and background', async () => {
      // Weapon is child on index 0, background on index 1
      const weaponChildIndex = 0;
      const backgroundChildIndex = 1;
      const weaponResId = weaponAssetsEquip[0]; // This asset is assigned to weapon first weapon
      await soldier.connect(addrs[0]).equip({
        tokenId: soldiersIds[0],
    ...
    it('cannot equip non existing child in slot (weapon in background)', async () => {
      // Weapon is child on index 0, background on index 1
      const badChildIndex = 3;
      const weaponResId = weaponAssetsEquip[0]; // This asset is assigned to weapon first weapon
      await expect(
        soldier.connect(addrs[0]).equip({
          tokenId: soldiersIds[0],
    ...
    it('cannot set a valid equippable group with id 0', async () => {
    ...
    it('cannot set a valid equippable group with part id 0', async () => {
    ...
    it('cannot equip into a slot not set on the parent asset (gem into soldier)', async () => {
      const soldierOwner = addrs[0];
      ...
      await expect(
        soldier.connect(addrs[0]).equip({
          ...
          assetId: soldierResId,
          slotPartId: partIdForWeaponGem,
          childAssetId: weaponGemAssetEquip,
        }),
    ...
    it('cannot equip wrong child in slot (weapon in background)', async () => {
      // Weapon is child on index 0, background on index 1
      const backgroundChildIndex = 1;
      const weaponResId = weaponAssetsEquip[0]; // This asset is assigned to weapon first weapon
      await expect(
    ...
    it('cannot equip child in wrong slot (weapon in background)', async () => {
      const childIndex = 0;
      const weaponResId = weaponAssetsEquip[0]; // This asset is assigned to weapon first weapon
      await expect(
        soldier.connect(addrs[0]).equip({
    ...
    it('cannot equip child with wrong asset (weapon in background)', async () => {
      const childIndex = 0;
      await expect(
        soldier.connect(addrs[0]).equip({
          tokenId: soldiersIds[0],
    ...
    it('cannot equip if not owner', async () => {
      // Weapon is child on index 0, background on index 1
      const childIndex = 0;
      const weaponResId = weaponAssetsEquip[0]; // This asset is assigned to weapon first weapon
      await expect(
    ...
    it('cannot equip 2 children into the same slot', async () => {
      // Weapon is child on index 0, background on index 1
      const childIndex = 0;
      const weaponResId = weaponAssetsEquip[0]; // This asset is assigned to weapon first weapon
      await soldier.connect(addrs[0]).equip({
      ...
      await expect(
        soldier.connect(addrs[0]).equip({
          tokenId: soldiersIds[0],
          childIndex: newWeaponChildIndex,
          assetId: soldierResId,
          slotPartId: partIdForWeapon,
          childAssetId: newWeaponResId,
        }),
      ).to.be.revertedWithCustomError(soldier, 'RMRKSlotAlreadyUsed');
    });

    it('cannot equip if not intented on catalog', async () => {
      // Weapon is child on index 0, background on index 1
      const childIndex = 0;
      const weaponResId = weaponAssetsEquip[0]; // This asset is assigned to weapon first weapon
    ...
    describe('With equipped children', async () => {
      ...
      it('can replace parent equipped asset and still unequip it', async () => {
        // Weapon is child on index 0, background on index 1
      ...
      it('can replace parent equipped asset and cannot not re-equip on top', async () => {
        // Weapon is child on index 0, background on index 1
        const newSoldierResId = soldierResId + 1;
        await soldier.addEquippableAssetEntry(
          newSoldierResId,
        ...
        await expect(
          soldier.connect(soldierOwner).equip({
            tokenId: soldierID,
            childIndex: weaponChildIndex,
            assetId: newSoldierResId,
            slotPartId: partIdForWeapon,
            childAssetId: weaponResId,
          }),
        ).to.be.revertedWithCustomError(soldier, 'RMRKSlotAlreadyUsed');
  ...
  describe('Unequip', async () => {
    it('can unequip', async () => {
      // Weapon is child on index 0, background on index 1
      const soldierOwner = addrs[0];
      const childIndex = 0;
      const weaponResId = weaponAssetsEquip[0]; // This asset is assigned to weapon first weapon
    ...
    it('can unequip if approved', async () => {
      // Weapon is child on index 0, background on index 1
      const soldierOwner = addrs[0];
      const childIndex = 0;
      const weaponResId = weaponAssetsEquip[0]; // This asset is assigned to weapon first weapon
    ...
    it('can unequip if approved for all', async () => {
      // Weapon is child on index 0, background on index 1
      const soldierOwner = addrs[0];
      const childIndex = 0;
      const weaponResId = weaponAssetsEquip[0]; // This asset is assigned to weapon first weapon
    ...
    it('cannot unequip if not equipped', async () => {
      await expect(
        soldier
          .connect(addrs[0])
          .unequip(soldiersIds[0], soldierResId, partIdForWeapon),
      ).to.be.revertedWithCustomError(soldier, 'RMRKNotEquipped');
    });

    it('cannot unequip if not owner', async () => {
      // Weapon is child on index 0, background on index 1
      const childIndex = 0;
      const weaponResId = weaponAssetsEquip[0]; // This asset is assigned to weapon first weapon
      await soldier.connect(addrs[0]).equip({
  ...
  describe('Transfer equipped', async () => {
    it('can unequip and transfer child', async () => {
      // Weapon is child on index 0, background on index 1
      const soldierOwner = addrs[0];
      const childIndex = 0;
      const weaponResId = weaponAssetsEquip[0]; // This asset is assigned to weapon first weapon
    ...
    it('child transfer fails if child is equipped', async () => {
      const soldierOwner = addrs[0];
  ...
  describe('Compose', async () => {
    it('can compose equippables for soldier', async () => {
      const childIndex = 0;
      const weaponResId = weaponAssetsEquip[0]; // This asset is assigned to weapon first weapon
      await soldier.connect(addrs[0]).equip({
        tokenId: soldiersIds[0],
    ...
    it('can compose equippables for simple asset', async () => {
      const allAssets = await view.composeEquippables(
        await background.getAddress(),
        backgroundsIds[0],
        backgroundAssetId,
    ...
    it('cannot compose equippables for soldier with not associated asset', async () => {
      const wrongResId = weaponAssetsEquip[1];
      await expect(
        view.composeEquippables(
          await weapon.getAddress(),
          weaponsIds[0],
          wrongResId,
        ),
  ...
  async function mintWeaponToSoldier(
    ...
    const newWeaponId = await nestMint(
      weapon,
      await soldier.getAddress(),
      soldierId,
    );
