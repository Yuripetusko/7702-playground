...
import { ADDRESS_ZERO, ONE_ETH, bn } from '../utils';
...
async function shouldHaveRoyalties(
  ...
  it('can get royalty data', async function () {
    ...
    expect(await this.token.royaltyInfo(tokenId, ONE_ETH * 10n)).to.eql([
      ADDRESS_ZERO,
      ONE_ETH,
    ]);
  });
  ...
  it('can get and update royalty receipient', async function () {
    ...
    expect(await this.token.getRoyaltyRecipient()).to.eql(
      await newRecipient.getAddress(),
    );
  });
  ...
  it('cannot update royalty receipient if not owner', async function () {
    ...
    await expect(
      this.token
        .connect(newRecipient)
        .updateRoyaltyRecipient(await newRecipient.getAddress()),
