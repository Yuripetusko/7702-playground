...
type GenericNestMintableNativeToken =
  ...
  | RMRKNestableLazyMintNative
  | RMRKNestableMultiAssetLazyMintNative;
type GenericMintableNativeToken =
...
async function mintFromMock(
  token: GenericMintableMock,
  to: string,
): Promise<bigint> {
...
async function getTokenIdFromTx(
...
async function mintFromErc20Pay(
  token: GenericMintableERC20Pay,
  to: string,
): Promise<bigint> {
...
async function mintFromNativeToken(
  token: GenericMintableNativeToken,
  to: string,
): Promise<bigint> {
...
async function nestTransfer(
  ...
  await token
    .connect(caller)
    .nestTransferFrom(caller.address, to, tokenId, parentId, '0x');
}
...
async function addAssetEntryFromImpl(
  token: Contract,
  data?: string,
): Promise<bigint> {
...
async function singleFixtureWithArgs(
  contractName: string,
  args: any[],
): Promise<Contract> {
